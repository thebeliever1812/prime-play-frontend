...
const FormInput = <T extends FieldValues>({ name, type = "text", label = "", placeholder = "", required = false, register, error }: FormInputProps<T>) => (
    <div className="w-full flex flex-col gap-1 items-start ">
        ...
        <div className={`w-full h-[35px] border-[1px] rounded-[10px] flex items-center px-3 cursor-pointer ${error && "border-red-600"}`}>
            <input id={name} type={type} placeholder={placeholder} {...register(name)} className="outline-none  w-full text-[14px] cursor-pointer" />
        </div>
